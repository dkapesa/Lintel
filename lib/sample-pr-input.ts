...
+}`,
  changePassport: normalizeChangePassport({
    ...
    claimedFiles: ["src/payments/refund-service.ts"],
    claimedSurfaces: ["Payments/domain logic", "External provider boundary"],
    claimedValidation: ["npm test -- refund-service"],
    assumptions: ["The gateway honours the existing idempotency key."],
    knownLimitations: ["No captured gateway sandbox result is attached to the change."],
