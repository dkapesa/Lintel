...
/* R5E.1D — the shared, presentational Human Decision content. Rendered
   identically inside the embedded Workspace panel (guided reach — see
   VerificationWorkspace.tsx's HumanDecisionPanel, R5E.1E.1 correction) and
   the manually activated dialog
   (docs/r5/R5E1A_SYSTEM_AND_INTERACTION_LOCK.md §12a: both surfaces "render
   every outcome unselected, keep the read-only boundary explicit, and
   record nothing"). All seven outcomes are the frozen product's own
   DECISION_OUTCOMES, rendered as truthful non-interactive records — never a
   disabled radio input — because selection is not implemented here and a
   disabled form control would misrepresent that (docs/r5/R5E1D task brief
   §8: "Do not use disabled radio buttons when that would create a
   misleading form"). Nothing here is a route, a form, or capable of
   recording, submitting or writing anything. Exported so both render sites
   share one implementation — R5E.1E.1 task brief §1: "Preserve one shared
   Human Decision content model. Do not create two unrelated
   implementations." */
export function HumanDecisionContent() {
  return (
...
/* R5E.1D — the manually activated dialog. Full dialog semantics:
   ...
   ("Read-only Human Decision"), never the guided embedded panel's own
   "scroll to continue, or open it directly" wording (R5E.1E.1: that label
   now lives in VerificationWorkspace.tsx's HumanDecisionPanel, not in a
   separate overlay component) — the visitor has already opened this dialog
   directly, so a label that invites further scrolling or opening would be
   false. The distinction is carried by component identity alone (this
   function only ever renders the manual label), so no new prop or state was
   introduced. */
