...
5. [LVOS Visual Calibration Note](./LVOS_VISUAL_CALIBRATION_NOTE.md) — subordinate execution guidance for refinement, spatial organisation and product presentation during LVOS-5 through V8.

The Visual Calibration Note does not amend LVOS v1.0. It translates the approved identity into an execution-quality threshold and records the agreed synthesis of operational workspaces, administrative discipline, split-pane inspection and product-led website presentation.
...
**Current programme update — 18 July 2026:** LVOS v1.0 and VC-1 through VC-4 are complete and closed foundations. **V8A through V8D — Experience and Motion** is complete on `v8-experience-and-motion`. [V8 Motion Constitution](./V8_MOTION_CONSTITUTION.md), [V8 Experience Storyboard](./V8_EXPERIENCE_STORYBOARD.md) and the [V8 final experience audit](./V8_FINAL_EXPERIENCE_AUDIT.md) record the approved landing, Workspace, Case File, reduced-motion and final QA result. V8 did not alter the fixed visual architecture. The next programme should be capability-led product work, initiated only by an explicit product decision. This update supersedes the historical sequencing statements below.
...
- LVOS-5, LVOS-6, LVOS-7 and V8 prompts must apply both LVOS v1.0 and the subordinate Visual Calibration Note.
