...
## V8B.1 implementation record â€” cinematic landing calibration

**Status:** Implemented; bounded landing calibration. No commit or push.

V8B.1 retains the VC-3 section ownership and V8B observer architecture. It corrects the opening-stage scale and sectional rhythm without introducing a new mockup, product route, motion framework or product fact. The hero is still one authentic Case File crop. At wide and compact desktop widths, its decorative outline rail yields the plane to the selected PR state, making PR #482, TESTS REQUIRED, risk 46/100 medium, F1/E1, C1 OPEN Â· BLOCKING and the pending Human Decision readable together. The same persistent identifiers and trace fragments continue through readiness, evidence, requirement, ledger and decision rather than repeating full headers.

The hero/readiness seam is shorter and remains a document seam, not a decorative transition. F1 stays the active observation, E1 stays visibly attached with DIRECTLY OBSERVED provenance, and the existing C1 consequence is now written at that point. The Merge Contract makes C1 the dominant engineering artifact: current E1 evidence is distinct from the exact clearance evidence, while counts, owner cue and unresolved state remain unchanged. The Verification Ledger becomes a deliberate single-column pause. Human Authority remains the strongest post-hero product scene, with recommendation context yielding to the neutral, unresolved human record. The final trace remains open and retains the existing next-case actions.
